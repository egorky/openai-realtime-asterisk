...
export function startOpenAISession(callId: string, ariClient: AriClientInterface, config: CallSpecificConfig): void {
  ...
  if (config.aiProvider === 'azure') {
    ...
    wsUrl = `${endpoint}/openai/realtime?api-version=${config.azureOpenAI.apiVersion}&deployment=${config.azureOpenAI.deploymentId}`;
    wsUrl = wsUrl.replace('http', 'ws');
    headers = { 'api-key': config.azureOpenAI.apiKey! };
    sessionLogger.info(`[${callId}] Connecting to Azure OpenAI Realtime WebSocket with URL: ${wsUrl}`);
  } else {
          ...
          */
          case 'response.done':
            ...
            if (session.processingToolCalls) {
              ...
              } else if (serverEvent.response?.status === 'incomplete') {
                msgSessionLogger.error(`[${callId}] OpenAI response incomplete. Resending last transcript.`);
                const call = (session.ariClient as AriClientService).activeCalls.get(callId);
                if (call && call.finalTranscription) {
                  requestOpenAIResponse(callId, call.finalTranscription, session.config);
                }
